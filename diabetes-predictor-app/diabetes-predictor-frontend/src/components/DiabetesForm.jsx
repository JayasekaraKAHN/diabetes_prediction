...
// Helper functions
const calculateObesityStatus = (bmi) => {
...
const checkPrediabetes = (hba1c, glucose) => {
  if (!hba1c && !glucose) return 0;
  // Prediabetes criteria
  if ((hba1c >= 5.7 && hba1c <= 6.4) || (glucose >= 140 && glucose <= 199)) return 1;
  return 0;
};
...
const DiabetesForm = ({ onSubmit, loading }) => {
  ...
  useEffect(() => {
    setFormData(prev => ({
      ...prev,
      obesity_status: calculateObesityStatus(prev.bmi),
      prediabetes: checkPrediabetes(prev.HbA1c_level, prev.blood_glucose_level)
    }));
  }, [formData.bmi, formData.HbA1c_level, formData.blood_glucose_level]);
  ...
  const sections = [
    ...
    { icon: Heart, title: 'Health Metrics', color: 'green' },
    ...
    >
          ...
          {sections.map((section, index) => (
            <button
              ...
              className={`flex items-center space-x-2 px-4 py-2 rounded-full transition-all ${
                activeSection === index ? 'bg-white/20 backdrop-blur-sm' : 'bg-white/10 hover:bg-white/15'
              }`}
            >
      ...
      <form onSubmit={handleSubmit} onKeyDown={handleKeyDown} className="p-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="col-span-1 md:col-span-2 grid grid-cols-2 gap-4 mt-4">
                    <label className="flex items-center space-x-2">
                      <input type="checkbox" name="hypertension" checked={!!formData.hypertension} onChange={handleCheckboxChange} className="h-5 w-5 text-orange-600"/>
                      <span className="text-gray-700 font-semibold">Hypertension</span>
                    </label>

                    <label className="flex items-center space-x-2">
                      <input type="checkbox" name="heart_disease" checked={!!formData.heart_disease} onChange={handleCheckboxChange} className="h-5 w-5 text-orange-600"/>
                      <span className="text-gray-700 font-semibold">Heart Disease</span>
                    </label>

                    <label className="flex items-center space-x-2">
                      <input type="checkbox" name="prediabetes" checked={!!formData.prediabetes} readOnly className="h-5 w-5 text-orange-600 cursor-not-allowed"/>
                      <span className="text-gray-700 font-semibold">Prediabetes</span>
                    </label>

                    <label className="flex items-center space-x-2">
                      <input type="checkbox" name="high_blood_pressure" checked={!!formData.high_blood_pressure} onChange={handleCheckboxChange} className="h-5 w-5 text-orange-600"/>
              ...
              {activeSection === 2 && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <input type="number" name="sleep_hours" value={formData.sleep_hours} onChange={handleChange} className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all" min="0" max="12" step="0.1" required/>
                  </div>

                  <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-3">Physical Inactivity</label>
                    <input type="checkbox" name="physical_inactivity" checked={!!formData.physical_inactivity} onChange={handleCheckboxChange} className="h-5 w-5 text-purple-600"/>
                  </div>

                  <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-3">Smoking History</label>
                    <select name="smoking_history" value={formData.smoking_history} onChange={handleChange} className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all">
                  ...
                  <div>
                    ...
                    <select name="alcohol_use" value={formData.alcohol_use} onChange={handleChange} className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all">
        ...
        <div className="flex justify-between mt-10">
          {activeSection > 0 && (
            <button type="button" onClick={handlePreviousSection} className="px-6 py-3 rounded-xl border border-gray-300 hover:bg-gray-100 transition-all">Previous</button>
          )}
          {activeSection < sections.length - 1 ? (
            <button type="button" onClick={handleNextSection} disabled={isNextDisabled()} className={`px-6 py-3 rounded-xl text-white transition-all ${isNextDisabled() ? 'bg-gray-400 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700'}`}>Next</button>
          ) : (
            <button type="submit" className={`px-6 py-3 rounded-xl bg-green-600 hover:bg-green-700 text-white flex items-center space-x-2 transition-all ${isSubmitDisabled() ? 'opacity-50 cursor-not-allowed' : ''}`} disabled={loading || isSubmitDisabled()}>
